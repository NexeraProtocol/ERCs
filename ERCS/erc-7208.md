...
An ODC MUST extend the functionality of **ERC721** through the incorporation of **Properties** in its internal storage. The **Properties** MAY have **Restrictions**.
...
Standardization: With the proliferation of various token standards on Ethereum (ERC20, ERC721, ERC1155), there is a need for a universal interface that can handle these token types uniformly. The Wrapper provides standardization, enabling a consistent approach to dealing with various token types, regardless of their implementation.

Composability and Interoperability: The Ethereum ecosystem thrives on the principle of 'money legos', where various DeFi components can interact and compose seamlessly. The Wrapper Component extends this principle by allowing different token types to be bundled together into a single entity (ODC). This increases the composability and interoperability between various protocols and platforms.

Efficient Asset Management: The Wrapper Component enables more efficient and flexible asset management. It allows users to bundle multiple assets into a single ODC, reducing the complexity of handling individual assets. Furthermore, the capability to 'unwrap' these bundled assets as needed, provides users with granular control over their digital assets.

Reducing Transaction Costs: By facilitating the bundling of multiple assets into a single ODC, the Wrapper Component can also help reduce transaction costs. Transferring or interacting with multiple individual tokens often leads to a high accumulation of gas fees. However, by wrapping these assets into a single entity, users can perform operations in a more cost-effective manner.
The Wrapper Component, through the aforementioned functionalities, seeks to address a spectrum of demands within the Ethereum community. By continually evolving and adapting the protocol, we strive to meet the dynamic needs of this ecosystem, thereby driving forward the Ethereum ethos of inclusivity, interoperability, and innovation.
...
The Metadata library includes base properties for ERC721 and ERC1155 tokens and allows for the addition of extra properties. These extra properties are flexible and extendable, covering string, date, integer, and decimal properties. This broad range of property types caters to the diverse metadata needs across different NFT projects.
