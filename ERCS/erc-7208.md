...
The Metadata library includes base properties for [ERC-721](erc-721.md) tokens and allows for the addition of extra **Properties**. These are flexible and extendable, covering `string`, date`, `integer`, and `decimal` **Properties**. This broad range of property types caters to the diverse metadata needs across different use cases.
